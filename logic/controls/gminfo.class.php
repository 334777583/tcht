	...
	*/
	public function getDetailInfo(){
		...
		if($gid) {
			...
			$rmb = D('chongzhi')->fquery("select sum(c_price*c_num) as rmb from chongzhi where c_pid=$gid and c_state=2");
			if($bo != '') {
				$data = $bo['seconddata'];
				...
				$play['recharge'] =	empty($rmb[0]['rmb'])?0:$rmb[0]['rmb'];
				$play['gold'] =	$bo['gold'];
				...
				$account = $point -> table('game_user')->field('account') -> where('id ='.$bo['accountid']) -> find();
				$forbidlogin = $point -> table('forbid_login') -> where('Account ="'.$account['account'].'"') -> find();
				if(!$forbidlogin){
	...
	public function getstarsInfo() {
		...
		if($gid) {
			$list = $point -> table('player_table')->field('StarDrom') -> where('guid=' . $gid) -> find();
			...
			$da1 = json_decode($data,true);
			$da = $da1['StarLifeLevelList'];
			if(is_array($da)){
