...
class singsevact{
	...
	public function SingSevAct() {
		$ip = get_var_value('ip');
		$obj = D('game'.$ip);
		$start = $obj ->field('u_date')->table('user')->order('u_date asc')->find();
		$startdate = get_var_value('startdate') == NULL ? date('Y-m-d',strtotime($start['u_date'])) : get_var_value('startdate');
		...
		if($ip) {
			...
			if(true) {	
				...
				// }else{
					...
					//游戏角色信息
					$sql = "SELECT p.AccountId,p.RoleName,p.level,p.CreateTime,p.LoginTime FROM player_table p LEFT JOIN game_user g ON g.id=p.AccountId WHERE p.LoginTime >= '".strtotime($startdate)."' and p.LoginTime <= '".$EndTime."' and p.level > 39 ORDER BY p.{$_POST['sort']} DESC";
					$SingResult = $point ->fquery($sql);	
				...
				echo json_encode(array('result' => $SingResult, 'startDate' =>$startdate , 'endDate' => $enddate));
				exit;
			} else {
				echo '1';
			}
