...
class usernew{
	...
	public function getKeepLV() {
		$ip = get_var_value('ip');
		$point = D('game'.$ip);
		$date = $point ->field('left(d_date,10) as d_date')->table('detail_login')->order('d_id asc')->find();
		$startdate = get_var_value('startdate') == NULL ? date('Y-m-d',strtotime($date['d_date'])) : get_var_value('startdate');
		if($ip && $startdate) {
			$result = array();			//统计结果
			...
			if($startdate < date("Y-m-d")) {
				...
				echo json_encode(array('result' => $data,'startDate'=>$startdate));
				exit;
			}else{
				echo '1';
			}	
	...
	public function getTaskLV() {
		...
		if (file_exists($taskfile)) {
			...
			fclose($fs);
			$task_json = json_decode($get_str,true);
				...
				if($list != '') {
					...
					if(!empty($result)) {			//组装数据
						...
						foreach($result as $level => $item) {
							...
							if(!empty($task_json[$level])){
								$data[] = array(
											'level' => $task_json[$level]['name'],
											'two_num' => $two_num,
											'two_per' => $two_per,
											'three_num' => $three_num, 
											'three_per' => $three_per,
