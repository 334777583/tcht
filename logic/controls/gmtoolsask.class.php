...
class gmtoolsask{
	...
	public function toolsAsk(){
		$fakermb = get_var_value('fakermb')==null?0:get_var_value('fakermb');
		$state = get_var_value('state')==null?1:get_var_value('state');
		$status = 1;
	...
	public function getAskTable(){
		...
		$list = $obj->table('tools_ask')-> where('t_status != -1 and t_ip = ' . $this->ip)->order('t_id desc')->limit(intval($page->getOff()),intval($this->pageSize))->select();
	...
	public function getToolDetail(){
		...
		$type2 = get_var_value('type2');
		$type3 = get_var_value('type3');
