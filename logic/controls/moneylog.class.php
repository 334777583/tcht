...
class moneylog{
	...
	public function __construct(){
		...
		$this->cointype = get_var_value('cointype');
	}
	...
	public function getHistoryLog(){
		...
		if($this->ip&&$this->codeValue&&$this->cointype) {
			global $t_conf;
			$srever = 's'.$this->ip;
			...
			$obj = D("game".$this->ip);
			$total =  $obj->table('water_log'.$tableNum)->where("date between '{$this->startDate}' and '{$this->endDate}' and ItemId=$this->cointype and playid=$this->codeValue")->total();
			
			$sql = "select a.Count,a.date,a.Source from water_log$tableNum as a ";
			$sql.= " where a.date between '{$this->startDate}' and '{$this->endDate}' and a.ItemId=$this->cointype and a.playid=$this->codeValue group by id limit ";
...
// 			-> limit(intval(($this->curPage-1)*$this->pageSize),intval($this->pageSize))->select();
	...
	public function getCurrentLog(){
		if ($this->ip&&$this->codeValue){
			...
			$moneylog = $this->getFileDate($file_type1,$this->codeValue,$this->cointype);
			if (!$moneylog){
				echo 1;exit;
			}
			foreach ($moneylog as $k=>$v){
	 ...
	 */
	public function getFileDate($path,$playid,$cointype){
		if (file_exists($path)) {
			...
			while(!feof($fp)) {
				...
				if(!empty($line)) {
					...
					if(is_array($arr)&&$arr['playid']==$playid&&$arr['ItemId']==$cointype) {
						$log_data[] = $arr;
					}
				}
			}
