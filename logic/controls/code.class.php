...
class code{
	...
	public function __construct(){
		...
		$this->cstatus = get_var_value('cstatus') == NULL? '' : get_var_value('cstatus');
	}
	...
	public function addCode(){
		...
		if ($this->ip==0){
			$result =  $obj -> table('gamedb') -> where(array('g_flag'=>1)) -> select();
		}else {
			$result =  $obj -> table('gamedb') -> where(array('g_flag'=>1, 'g_id' => $this->ip)) -> select();
		}
		...
		foreach ($result as $sKey=>$sVal){
			...
			foreach($code_arr as $k => $code){
				...
				$insert = $obj->table('new_gift')->insert($li[0]);
			}
			
		}
		
		if(isset($list) && count($list) > 0){
			$tmpfname = tempnam(ServicePath.'excel','ASDFGHJKEWRTYUI');
	...
	public function getCode(){
		...
		if(!empty($this->cstatus)){
			if($this->cstatus == 0){
				$where .= 'playerguid < 0 and ';
		...
		foreach($st as $key => $value){
            ...
            $list[$key]['playerguid'] = $value['playerguid'];
	...
	**/
	public function excel(){
		...
		if(!empty($list)){
			...
			$objPHPExcel->getActiveSheet()->setCellValue('C1', '服务器');
			$objPHPExcel->getActiveSheet()->setCellValue('D1', '激活码ID');
			$objPHPExcel->getActiveSheet()->setCellValue('E1', '使用时限');
			$objPHPExcel->getActiveSheet()->setCellValue('F1', '生成时间');
			
			if (is_array($list)) {
				foreach($list as $k => $item){
					$objPHPExcel->getActiveSheet()->setCellValue('A'.($k+2), $item["id"]); 
					$objPHPExcel->getActiveSheet()->setCellValue('B'.($k+2), $item["type"]);
					$objPHPExcel->getActiveSheet()->setCellValue('C'.($k+2), $item["ip"]);
					$objPHPExcel->getActiveSheet()->setCellValue('D'.($k+2), $item["item_id"]);
					$objPHPExcel->getActiveSheet()->setCellValue('E'.($k+2), $item["end_time"]);
					$objPHPExcel->getActiveSheet()->setCellValue('F'.($k+2), $item["instime"]);
				}	
			}	
...
// 			$objWriter->save($f.".xlsx");
// 			header('Content-type: application/vnd.ms-excel');//输出的类型
// 			header('Content-Disposition: attachment; filename="激活码.xlsx"'); //下载显示的名字,注意格式
// 			ob_clean();
// 			flush();
// 			readfile($f.".xlsx");
	...
	**/
	/*public function excel(){
		...
		$list = $point ->table('activity_gift')->where(array('server_id ' => $ip ))->select();
		if(!empty($list)){
			...
			$objPHPExcel->setActiveSheetIndex(0);	
			$objPHPExcel->getActiveSheet()->setCellValue('A1', '礼包ID'); 
			$objPHPExcel->getActiveSheet()->setCellValue('B1', '激活码');
			$objPHPExcel->getActiveSheet()->setCellValue('C1', '激活类型');
			$objPHPExcel->getActiveSheet()->setCellValue('D1', '服务器');
			$objPHPExcel->getActiveSheet()->setCellValue('E1', '使用时限');
			$objPHPExcel->getActiveSheet()->setCellValue('F1', '使用情况');
			
			if (is_array($list)) {
				foreach($list as $k => $item){
					if($item['playerguid'] < 0){
						$use = '未使用';
					}else{
						$use = '已使用';
					}
					$objPHPExcel->getActiveSheet()->setCellValue('A'.($k+2), $item["gift_item_id"]); 
					$objPHPExcel->getActiveSheet()->setCellValue('B'.($k+2), $item["sn_code"]);
					$objPHPExcel->getActiveSheet()->setCellValue('C'.($k+2), $item["pack_type"]);
					$objPHPExcel->getActiveSheet()->setCellValue('D'.($k+2), $item["server_id"]);
					$objPHPExcel->getActiveSheet()->setCellValue('E'.($k+2), $item["end_time"]);
					$objPHPExcel->getActiveSheet()->setCellValue('F'.($k+2), $use);
				}	
			}	
		...
		$list = $point ->table('activity_gift')->where(array('server_id ' => $ip ))->select();
			foreach($list as $k => $item){
				...
				if($item["end_time"] == '0'){
					...
					$list[$k]["time"] = $item["end_time"];
				}
				
				$str = "礼包ID\t激活码\t激活码类型\t服务器\t激活码状态\t使用时间\r\n";
				$str .= $list[$k]["gift_item_id"] ."\t";
				$str .= $list[$k]["sn_code"] ."\t";
				$str .= $list[$k]["pack_type"] ."\t";
				$str .= $list[$k]["server_id"]."\t";
				$str .= $list[$k]["used"] ."\t";
				$str .= $list[$k]["time"] ."\r\n";
			}
