...
class DB
{
	...
	protected function connect()
	{
		...
		if (mysqli_connect_errno()) {
			writeFile("Connect failed: ".mysqli_connect_error());
		}else {
	...
	protected function query($sql,$handle){
		//writeFile('['.date('Y-m-d H:i:s').'] [SQL:'.$handle.']'."\r\n");
		$this->clearfield();
	...
	protected function returnSql()
	{
		...
		if($db_result){
			...
			writeFile('SQL Error <br>'.$this->sql .'<br>  Error : '.$this-> obj -> error);
	...
	{    
		...
		if(!$result){
			...
			writeFile("SQL Error :" . $this -> sql."   Error : ".$this-> obj -> error);
			return false;
		}else{
			return true;	
		}
	...
	private function setTable(){
		...
		if($result){
			...
			writeFile("SQL ERROR :desc {$this->tabName}  Error :".$this-> obj -> error);
		}
		if (empty($fields)){
			$this->fields = '*';
		}else {
	...
	public function __call($name, $arguments){
		writeFile('your call function '.$name.' and arguments '.$arguments.' is not exist ');
	}
