...
class NewGameController extends CommonController {
	public function _initialize() {
		// 判断用户权限
		if (! in_array ( '00501200', session ( 'user_code_arr' ) )) {
	...
	public function index() {
		$serverList = M ( 'gamedb' )->where ( array (
				'g_flag' => 1 
		) )->select ();
	...
	public function dbEditor() {
		...
		$data ['g_ip'] = I ( 'ip' );
		$data ['g_name'] = I ( 'name' );
		$data ['g_domain'] = I ( 'domain' );
		$status = M ( 'gamedb' )->save ( $data );
		if ($status) {
			// 操作记录
			action_log ( session ( 'username' ), 0, 11, '修改服务器（' . $data ['g_id'] . '）' );
			$this->ajaxReturn ( array (
					'status' => 1,
					'info' => '修改成功' 
			) );
	...
	public function dbDelete() {
		...
		if ($status) {
			// 操作记录
			action_log ( session ( 'username' ), 0, 11, '删除服务器（' . $gid . '）' );
			$this->ajaxReturn ( array (
					'status' => 1,
					'info' => '删除成功' 
			) );
	...
	public function dbAdd() {
		...
		$data ['g_domain'] = I ( 'domain' );
		$data ['g_flag'] = 1;
		$data ['g_inserttime'] = date ( 'Y-m-d H:i:s', NOW_TIME );
		$status = M ( 'gamedb' )->add ( $data );
		...
		// 操作记录
		action_log ( session ( 'username' ), 0, 11, '添加服务器（' . $status . '）' );
		
		$status1 = M ( "servers" )->add ( array (
				"s_name" => $data ['g_name'],
				"s_ip" => $data ['g_ip'],
				"s_gid" => $status,
				"s_port" => '20101',
				"s_domain" => $data ['g_domain'],
				"s_inserttime" => date ( "Y-m-d H:i:s", NOW_TIME ) 
		) );
		
		// 操作记录
		action_log ( session ( 'username' ), 0, 10, '添加服务器（' . $status1 . '）' );
		
		$connect = mysql_connect ( C('DB_HOST'), C('DB_USER'), C('DB_PWD') );
		$newDB = C ( 'GAME_ANALYSIS_PREFIX' ) . $status;
		$sql = 'CREATE DATABASE ' . $newDB . ' COLLATE=utf8_unicode_ci;';
		$status2 = mysql_query ( $sql, $connect );
		if (! $status2) {
			$this->ajaxReturn ( array (
					'status' => 0,
					'info' => '创建数据库失败' 
